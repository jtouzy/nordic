...
describe('QueryBuilder.getInsertQuery', () => {
  ...
  it('Should generate SQL INSERT query with timestamped columns for insert', () => {
  ...
  it('Should generate SQL INSERT query with timestamped columns for insert without base order', () => {
    ...
    expect(insert.text).to.be.equal('INSERT INTO secured.articles (title, creation_date, tokens, reference) VALUES ($1, now(), $2, $3) RETURNING *')
    expect(insert.values).to.be.eql(['Toto', 'myToken', 1])
  })
  it('Should generate SQL INSERT query with timestamped columns for insert and update', () => {
    const sut = new QueryBuilder({ name: 'articles', schema: 'secured' }, null, { insert: 'creation_date', update: 'updated_date' })
    const insert = sut.getInsertQuery([{ title: 'Toto', tokens: 'myToken', reference: 1 }])
    expect(insert.text).to.be.equal('INSERT INTO secured.articles (title, tokens, reference, creation_date) VALUES ($1, $2, $3, now()) RETURNING *')
    expect(insert.values).to.be.eql(['Toto', 'myToken', 1])
  })
  it('Should generate SQL INSERT query with properties mapping and timestamped columns for insert', () => {
  ...
  it('Should generate SQL INSERT query with multiple inserted values, and properties mapping, and timestamped columns', () => {
    const sut = new QueryBuilder({ name: 'articles', schema: 'secured' }, { tokens(item, value) { return `to_tsvector(${value})` } }, { insert: 'creation_date' })
    const insert = sut.getInsertQuery([{ title: 'Toto', reference: 1, tokens: 'myToken' }, { title: 'Titi', reference: 2, tokens: 'myToken2' }])
...
describe('QueryBuilder.getUpdateQuery', () => {
  ...
  it('Should generate SQL UPDATE query with NULL conditions', () => {
    const sut = new QueryBuilder({ name: 'articles', schema: 'secured' })
    const update = sut.getUpdateQuery({ title: null })
    expect(update.text).to.be.equal('UPDATE secured.articles SET title = $1 RETURNING *')
    expect(update.values).to.be.eql([null])
  })
  it('Should generate SQL UPDATE query with undefined conditions', () => {
    const sut = new QueryBuilder({ name: 'articles', schema: 'secured' })
    const update = sut.getUpdateQuery({ title: undefined })
    expect(update.text).to.be.equal('UPDATE secured.articles SET title = $1 RETURNING *')
    expect(update.values).to.be.eql([null])
  })
  ...
  it('Should generate SQL UPDATE query with timestamped columns for update', () => {
  ...
  it('Should generate SQL UPDATE query with timestamped columns for update without base order', () => {
    const sut = new QueryBuilder({ name: 'articles', schema: 'secured' }, null, { update: 'updated_date' })
    const update = sut.getUpdateQuery({ title: 'Toto', updated_date: 'value', tokens: 'myToken' })
    expect(update.text).to.be.equal('UPDATE secured.articles SET title = $1, updated_date = now(), tokens = $2 RETURNING *')
    expect(update.values).to.be.eql(['Toto', 'myToken'])
  })
  it('Should generate SQL UPDATE query with timestamped columns for insert & update with updated columns related to timestamped columns', () => {
    const sut = new QueryBuilder({ name: 'articles', schema: 'secured' }, null, { insert: 'creation_date', update: 'updated_date' })
    const update = sut.getUpdateQuery({ title: 'Toto', tokens: 'myToken', creation_date: 'value' })
    expect(update.text).to.be.equal('UPDATE secured.articles SET title = $1, tokens = $2, creation_date = $3, updated_date = now() RETURNING *')
    expect(update.values).to.be.eql(['Toto', 'myToken', 'value'])
  })
  it('Should generate SQL UPDATE query with timestamped columns for update with conditions', () => {
    const sut = new QueryBuilder({ name: 'articles', schema: 'secured' }, null, { update: 'updated_date' })
    const update = sut.getUpdateQuery({ title: 'Toto', tokens: 'myToken' }, { id: 'identifier' })
    expect(update.text).to.be.equal('UPDATE secured.articles SET title = $1, tokens = $2, updated_date = now() WHERE id = $3 RETURNING *')
    expect(update.values).to.be.eql(['Toto', 'myToken', 'identifier'])
  })
  it('Should generate SQL UPDATE query with timestamped columns for update with null updated dates', () => {
    const sut = new QueryBuilder({ name: 'articles', schema: 'secured' }, null, { update: 'updated_date' })
    const update = sut.getUpdateQuery({ title: 'Toto', tokens: 'myToken', updated_date: null })
    expect(update.text).to.be.equal('UPDATE secured.articles SET title = $1, tokens = $2, updated_date = now() RETURNING *')
    expect(update.values).to.be.eql(['Toto', 'myToken'])
  })
  it('Should generate SQL UPDATE query with properties mapping and timestamped columns for update', () => {
